...
function convertPkcs1ToPkcs8(pkcs1Buffer) {
...
async function signRs256(unsignedToken, pem) {
...
export async function createAppJwt(appId, pem, now = Math.floor(Date.now() / 1000)) {
    if (!appId) {
